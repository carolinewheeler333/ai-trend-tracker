...
/* ── Column divider ── */
.divider { border-left: 1px solid #C8B99A; min-height: 500px; margin-top: 50px; }
...
# ── Source colours (dark, readable on cream) ─────────────────────────────────
SOURCE_COLOURS = {
    "ArXiv AI":        "#1E3A8A",
    "ArXiv ML":        "#1E3A8A",
...
if digest:
    ...
    <div class="ticker-wrap">
      ...
      <span class="ticker-scroll">{ticker}</span>
    </div>
    """, unsafe_allow_html=True)
...
if briefings.get("industry") or briefings.get("research"):
    st.markdown('<div class="section-label">Today\'s Briefing</div>', unsafe_allow_html=True)
    b1, gap, b2 = st.columns([10, 1, 10])
    with b1:
        if briefings.get("industry"):
            st.markdown(f"""
            <div class="briefing">
              <div class="briefing-eyebrow">Industry</div>
              <div class="briefing-body">{briefings['industry']}</div>
            </div>
            """, unsafe_allow_html=True)
    with gap:
    ...
    with b2:
        if briefings.get("research"):
            st.markdown(f"""
            <div class="briefing">
              <div class="briefing-eyebrow">Research</div>
              <div class="briefing-body">{briefings['research']}</div>
            </div>
            """, unsafe_allow_html=True)
...
with f1:
    selected = st.multiselect("Sources", all_sources, default=all_sources,
                              label_visibility="collapsed", key="source_filter")
with f2:
    search = st.text_input("Search", placeholder="Search headlines...",
                           label_visibility="collapsed", key="search_input")

industry_f = [a for a in industry if a["source"] in selected
              and (not search or search.lower() in a["title"].lower())]
research_f = [a for a in research if a["source"] in selected
              and (not search or search.lower() in a["title"].lower())]
...
# ── Footer ────────────────────────────────────────────────────────────────────
count = digest.get("article_count", len(articles))
st.markdown(f"""
<div class="paper-footer">
  The AI Times &nbsp;·&nbsp; {count} articles &nbsp;·&nbsp; Updated daily 07:00 UTC &nbsp;·&nbsp; Summaries by LLaMA 3
</div>
""", unsafe_allow_html=True)
